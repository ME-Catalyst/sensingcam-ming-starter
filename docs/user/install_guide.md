...
4. **Review Frigate** – Edit `src/frigate/config.yml` with RTSP URLs, recording retention, and hardware acceleration parameters.
5. **Start Stack** – `docker compose -f src/docker-compose.yml up -d`.
6. **Verify** – Run `scripts/verify_stack.sh` to check service health.
7. **Exercise Camera** – Set `SICK_CAMERA_HOST` and execute `scripts/test_camera_api.sh` to confirm REST connectivity.
